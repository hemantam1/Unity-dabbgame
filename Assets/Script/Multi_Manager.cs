...
public class Multi_Manager : MonoBehaviour
{
    ...
    public void Transmit_DisplaySkipM(int val)
    {
        if (FST_Gameplay.IsMultiplayer)
            Pview.RPC("Recieve_DisplaySkipM", RpcTarget.Others, val);
    }

    [PunRPC]
    void Recieve_DisplaySkipM(int val)
    {
    ...
    {
        if (FST_Gameplay.IsMultiplayer)
            Pview.RPC("SetNameMulti", RpcTarget.All);
    }
    ...
    public void SetNameMulti()
    {
        ...
        foreach (Photon.Realtime.Player ply in PhotonNetwork.PlayerList)
        {
    ...
    public void MultiplayerDiceClicked()
    {
        if (FST_Gameplay.IsMultiplayer)
            Pview.RPC("AnimationPlay", RpcTarget.All);
    }
    ...
    public void AnimationPlay()
    {
        ...
        Playarea.instt.ReceiveDiceClick();
    }

    public void SetDiceValmultiplayer(int val)
    {
        if (FST_Gameplay.IsMultiplayer)
            Pview.RPC("DicevalSet_Multiplayer", RpcTarget.All, val);
    }
    ...
    public void Multiplayer_turn_pass(int turn)
    {
        if (FST_Gameplay.IsMultiplayer)
            Pview.RPC("TurnPass_Multiplayer", RpcTarget.Others, turn);
    }

    [PunRPC]
    public void TurnPass_Multiplayer(int turn)
    {
        //  if(PhotonNetwork.IsMasterClient)
        Playarea.instt.ReceiveTurn(turn);
    }

    public void Multiplayer_skipBt()
    {
        if (FST_Gameplay.IsMultiplayer)
            Pview.RPC("Skipppp", RpcTarget.Others);
    }
    ...
    {
        if (FST_Gameplay.IsMultiplayer)
            Pview.RPC("Skip_Multiplayer", RpcTarget.All, skip_no, val);
    }
    ...
    {
        if (FST_Gameplay.IsMultiplayer)
            Pview.RPC("Undo_multiplayer", RpcTarget.All);
    }
    ...
    {
        if (FST_Gameplay.IsMultiplayer)
            Pview.RPC("PlayerMove_multi", RpcTarget.All, indx_no, CurrDiceVal);
        // PhotonNetwork.("")
    }
    ...
    {
        Pview.RPC("SafeZone_Multiplayer", RpcTarget.All, one, two, three, four, five);
    }
    ...
    {
        Pview.RPC("Ques_Multiplayer", RpcTarget.All, one, two, three, four, five);
    }
    ...
    public void sendmesg(string nm, string msg)
    {
        ...
        Pview.RPC("messagesend", RpcTarget.All, nm, msg, LangNumber);
    }
    ...
    public void imojisimg(string nm, int emono)
    {
        ...
        Pview.RPC("emmosjiesno", RpcTarget.All, nm, emono, LangNumber);
    }
    ...
    {
        Pview.RPC("changePoss", RpcTarget.Others, p1_pno, p1_pos, p2_pno, p2_pos);
    }
    ...
    {
        Pview.RPC("PutBakPoss", RpcTarget.Others, p1_pno, p1_pos);
    }
    ...
    {
        Pview.RPC("OpenAlertCardWithMsg", RpcTarget.Others, msg);
    }
    ...
    {
        Pview.RPC("SkipOnePlayerTurn", RpcTarget.Others, val);
    }
    ...
    {
        Pview.RPC("RollForPlayer", RpcTarget.Others, val);
    }
    ...
    {
        Pview.RPC("CardRandomValIs", RpcTarget.All, val);
    }
