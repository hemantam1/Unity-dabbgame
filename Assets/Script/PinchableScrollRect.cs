...
public class PinchableScrollRect : ScrollRect
{
	...
	[SerializeField] float _zoomLerpSpeed = 3f;
	...
	private void Update ()
	{
		...
		//pc input end
		if (Mathf.Abs (content.localScale.x - _currentZoom) > 0.001f) {  //&& content.localScale.x >= 1 && content.localScale.x <= 5
